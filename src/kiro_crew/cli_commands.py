...
def _handle_workspace(args: argparse.Namespace) -> None:
    ...
    if action == "list":
        ...
        if copy_from:
            ...
            # Composed the way _ws_dir_resolves_inside_home judges it (``~``
            # expanded, an absolute dir taken as given) and resolved once here,
            # exactly like the plain branch: the install and the fallback mkdir
            # below both land on this path, so it must be the directory validation
            # looked at, not ``<home>/~/...``.
