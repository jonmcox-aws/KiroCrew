...
class A2aAgentConfig:
    ...
    agent_card_url: str = field(
        ...
        metadata=_meta(
            ...
            "is read from the card; the provider fetches it on start(). Must be "
            "https://: task text and credentials travel only over TLS. A plain-http "
            "loopback origin (a local shim, the test fixture) is admitted only when "
            "the gateway's environment names it in KIROCREW_A2A_PLAINTEXT_ORIGINS.",
        ),
    )
