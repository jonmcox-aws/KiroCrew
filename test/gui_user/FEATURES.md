...
| Chat sessions (`chat`) | 37 | 4 | 25 | 1 | 3 | 4 |
| Sessions sidebar & folders (`sidebar`) | 29 | 6 | 21 | 1 | 1 | 0 |
| Crew Members (`members`) | 2 | 0 | 2 | 0 | 0 | 0 |
| Settings (`settings`) | 107 | 19 | 66 | 6 | 11 | 5 |
| Apps & App Store (`apps`) | 31 | 4 | 16 | 2 | 7 | 2 |
| Schedule (cron jobs) (`schedule`) | 10 | 2 | 4 | 0 | 0 | 4 |
| Knowledge library (`knowledge`) | 8 | 1 | 6 | 0 | 0 | 1 |
| Artifacts (`artifacts`) | 11 | 1 | 6 | 0 | 3 | 1 |
...
| Onboarding & login (`onboarding`) | 5 | 1 | 4 | 0 | 0 | 0 |
| Search everywhere & command palette (`search`) | 1 | 1 | 0 | 0 | 0 | 0 |
| Developer tools (`developer`) | 14 | 0 | 13 | 0 | 0 | 1 |
...
|---|---|---|---|---|
| native-only | `popout-chat` | chat | Popout chat window | Opening an OS window is Electron behaviour, so native-only; degradable to a web-only check by loading /popout/chat/<slug> directly and asserting the chrome-less frame. Two popout records merged. |
| native-only | `popout-terminal-window` | sidebar | Pop the terminal out and re-dock it | Requires a second window and a PTY; Electron-first, so native-only. Degradable: load /popout/terminal directly and assert the frame. Moved the feature-map popout-terminal record from the chat bucket (docked terminal is … |
| native-only | `computer-use-live-view-pip` | settings | Live view PiP mirrors screenshots the agent already read | Depends on a macOS/Windows accessibility capture and a model that invokes computer_get_state; neither exists on the Linux fake-ACP target. |
| native-only | `settings-channels-imessage-configure` | settings | Configure iMessage channel (enable, allowed handles, service, DB path) | Requires the macOS Messages database; form rendering is degradable to web-only but the channel cannot start on Linux. |
...
| native-only | `settings-computer-use-enable-toggle` | settings | Enable computer use and tune screenshot/tree options | Persistence is web-testable (degradable) but the capability only functions with the desktop accessibility layer. |
...
| needs-secret | `chat-channel-mirroring` | chat | Channel mirroring | Linking requires a real messaging-provider token to list channel targets; the menu shell alone renders without one. |
...
| needs-secret | `connections-services-gallery-oauth` | settings | Connect a third-party service from the Services gallery | Completing OAuth needs a real provider account; card rendering alone is nightly-testable. |
| needs-secret | `settings-channel-connect-slack` | settings | Connect Slack in the Slack channel panel | Requires real bot tokens; form rendering is nightly-testable. |
| needs-secret | `settings-channels-feishu-wecom-configure` | settings | Configure Feishu / WeCom channels | Requires real platform app credentials to verify a connection. |
| needs-secret | `settings-channels-teams-configure` | settings | Configure Microsoft Teams channel (App ID, password, tenant, allowed users) | Requires real Azure Bot credentials to reach a connected state. |
| needs-secret | `settings-channels-webex-configure` | settings | Configure Webex channel (token, allowed emails, group spaces, thread reply) | Connection verification requires a real Webex bot token; form is renderable on nightly. |
| needs-secret | `settings-channels-weixin-configure` | settings | Configure WeChat (Weixin) channel | Connecting requires a real WeChat login/QR; UI-only form is nightly-degradable. |
| needs-secret | `settings-channels-whatsapp-configure` | settings | Configure WhatsApp channel (enable, who can message, how the agent joins in) | Real account pairing required for connection; conditional form rendering could be nightly. |
| needs-secret | `settings-instances-remote-crew` | settings | Add a remote instance and switch panes via the header tab strip | Connecting needs a real peer and tunnel credentials; the form and empty tab strip are nightly-testable. |
| needs-secret | `shell-instances-tab-bar-switching` | settings | Switch between Local and remote instance panes | Needs a reachable remote instance over a tunnel. |
| needs-secret | `themes-install-pack-from-github` | settings | Install a theme pack from a github.com URL | Needs outbound network to github.com (and a sandbox backend), unavailable on the target. |
| needs-secret | `wakatime-activity-and-export` | settings | WakaTime coding activity and export | Stats come from the WakaTime API with a personal key; the empty-state alone is nightly-renderable. |
...
| excluded | `chat-remote-bound-session` | chat | Remote-bound session | This is not a missing credential but a missing second live Kiro Crew instance plus a network tunnel, which the Xvfb+fake-ACP, no-network target cannot provide. Only the flag-gated menu entry is renderable (cover that in… |
...
| excluded | `kas-auth-chat-error-row-sign-in-link` | chat | An auth-required turn renders a 'Sign in to Kiro' error row deep-linking to the card | Excluded: the fake ACP backend cannot emit the AcpAuthRequired vocabulary (only [[ERROR]]), so the row and its deep link are unreachable on the target. Closing that harness gap (an auth-required sentinel) is the prerequ… |
...
| excluded | `standalone-cloud-launch` | settings | Cloud launch | Kept excluded rather than needs-secret: beyond real AWS credentials it provisions billable infrastructure and exposes Stop/Delete-by-tag lifecycle, so it must not run even with secrets. Only the lane selector and IAM po… |
...
| excluded | `schedule-agent-chat-egress` | schedule | Agent chat egress | No UI and requires a live provider; not pixel-testable. |
| excluded | `schedule-session-control` | schedule | Session control | No UI; side effects show up as ordinary sessions covered elsewhere. |
| excluded | `schedule-session-ledger` | schedule | Session ledger | No UI surface; not pixel-testable. |
| excluded | `schedule-work-ledger` | schedule | Work ledger | No UI surface today; not pixel-testable. |
...
| excluded | `standalone-openai-compatible-api` | developer | OpenAI-compatible API | HTTP-only surface with no dashboard pixels. |
...
| Slug | Title | Why |
|---|---|---|
| `capabilities` | Agent Capabilities | Crews, Agent Templates, Connections, Skills, Prompts, Steering, Hooks and Workflows share the /capabilities page and are neither settings nor knowledge; they were mapped to 'settings' as the closest fit. |
| `memory` | Memory, lessons and usage | Memory browser, episodic search, embeddings, usage and portability are a distinct area under Settings → Overview but not settings themselves; mapped to 'settings'. |
| `task-runner` | Task Runner | Task Runner, task detail and spec refinement live under /projects in the Apps rail group but are a first-party surface; mapped to 'apps'. |
| `terminal` | Terminal panel | Docked and per-chat PTY terminals and the terminal popout are mapped to 'chat' for lack of a terminal feature. |
| `popout` | Popouts and embeds | Separate chrome-less route trees (/popout/*, /embed/*) were mapped to chat/artifacts/settings per content; a dedicated slug would group them. |
| `remote-instances` | Remote instances and cloud launch | Instances settings, cloud launch and remote-bound sessions form one remote-crew area; mapped to 'settings' and 'chat'. |
| `navigation` | Routing and redirects | Legacy redirect rows and the rail-pinning preference are navigation concerns mapped to 'sidebar' as the closest fit. |
| `api` | Headless API surfaces | Session ledger, work ledger, session control, agent chat egress and the OpenAI-compatible shim have no UI; mapped to 'schedule' or 'developer' and marked excluded. |
| `topbar` | Top bar (search trigger, readout capsule, feedback pill, bell, instance tabs) | Several records (readout capsule, feedback pill, instance tab bar, update pill) live in the header rather than any of the listed features; currently mapped to 'settings' or 'chat' as closest fit. |
| `connections` | MCP servers and third-party service connections | pages/connections/* is its own surface with a provider registry and OAuth relay; mapped to 'settings' for lack of a dedicated slug. |
| `webhooks` | Inbound webhooks page | /webhooks is a preview-gated standalone surface with its own registry entry; mapped to 'settings' because it is reached via the Settings tab. |
| `instances` | Multi-instance (remote crew) management and pane switching | InstanceTabBar, InstancesViewport, and Settings > Remote Instances form a distinct surface; mapped to 'settings'. |
| `worlds` | Worlds 3D scenes | WorldsPage and pages/scenes/* are a builtin app with WebGL rendering; mapped to 'apps'. |
| `dev-fleet` | Dev Fleet worktree and pod management | DevFleetPage is a builtin developer surface distinct from the Developer internals page; mapped to 'developer'. |
| `channels` | Chat channel integrations | Slack/Discord/Telegram/Webex/Teams/WhatsApp/WeChat/iMessage/Feishu/WeCom configuration is a large sub-tree under /settings/channels with its own connect/status lifecycle; grouping under 'settings' hides it. |
| `security` | Security & governance | Settings > Security has eight sub-sections (posture, approval, rules, tailnet, apps, layers, governance, docs) plus mobile login; it behaves like its own feature area distinct from generic preferences. |
| `side-panel` | Side Panel Tabs | Changes/Artifacts/Files pinned tabs, + menu views, Side Chat, terminal and browser tabs form a distinct surface currently filed under chat. |
| `computer-use` | Computer Use | Desktop GUI automation has its own settings panel, keystone enable, live view and platform gating; filed under settings for now. |
| `themes` | Themes & Display | Theme packs (install, levels, persona consent) and display language are a self-contained subsystem filed under settings. |
| `auth` | Authentication & Sign-in | Dashboard token gate and Kiro (KAS) sign-in card are login surfaces, not settings per se. |
