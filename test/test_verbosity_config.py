...
class TestAnswerOnlyBlock:
    ...
    Measured on 30 one-shot replies (5 asks x 3 modes x 2 runs): same word
    count as the essay, reading grade 7 -> 2, and a picture on 4 of the 5 asks
    that had a shape where the essay drew none.
    """
    ...
    def test_shape_check_draws_the_shape_in_the_form_the_surface_renders(self):
        ...
        gated on the surface: a widget only where the Inline Widgets section
        is present (dashboard, density permitting), else mermaid, else a
        table -- an unconditional "emit a widget" would land raw ``<mcwidget>``
        markup in a Slack or CLI reply.
        """
        block = self._block()
        assert "Does the answer have a shape" in block
        assert "steps, before/after, cases and verdicts, sizes" in block
        assert "a widget when your instructions carry an Inline Widgets section" in block
        assert "(load `widgets` first)" in block
        assert "else a mermaid fence, else a plain table" in block
    ...
    def test_cut_check_names_what_goes_and_what_stays(self):
        ...
        """
        block = self._block()
        assert "Delete: preamble, what you did, why, options you rejected, caveats, offers" in block
        assert "code, commands and paths verbatim" in block
        assert "required formats ([OPTIONS:], diffs, PR links)" in block
