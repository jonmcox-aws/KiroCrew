...
long as the shipped renderer reads a crew-record cue kind-agnostically. A ghost's `expressions` (per-state eyes/mouth) still round-trips too, for as long
as the shipped renderer draws it. A `motions` or `expressions` pick named on a
